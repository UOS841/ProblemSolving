...
double square_distance(const Star &a, const Star &b) {
...
int main() {
	...
	for (int a = 0; a < n; a++) {
		for (int b = a + 1; b < n; b++) {
			edges.push_back({
				&stars[a],
				&stars[b],
				square_distance(stars[a], stars[b])
			});
		}
	}
	...
	sort(edges.begin(), edges.end(), [](const Edge &a, const Edge &b) {
		return a.square_cost < b.square_cost;
	});

	// select edges
	double sum_square_cost = 0;
	{
		int n_selected_edges = 0;
		for (const Edge &edge : edges) {
			if (!unite(*edge.a, *edge.b)) continue;
			n_selected_edges++;
			sum_square_cost += edge.square_cost;
			if (n_selected_edges == n - 1) break;
		}
	}

	// print the sum of cost
	cout << sqrt(sum_square_cost);
